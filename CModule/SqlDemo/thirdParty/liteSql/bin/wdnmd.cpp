...
const int userInfo::SexType::Male(1);
const int userInfo::SexType::Female(0);
userInfo::SexType::SexType(const std::string& n, AT_field_type t, const std::string& tbl, const litesql::FieldType::Values& vals)
         : litesql::FieldType(n,t,tbl,vals) {
}
const int userInfo::Sex::Male(1);
const int userInfo::Sex::Female(0);
...
std::vector < std::pair< std::string, std::string > > userInfo::sex_values;
const litesql::FieldType userInfo::Sex("sex_",A_field_type_integer,table__);
void userInfo::initValues() {
    sex_values.clear();
    sex_values.push_back(make_pair<std::string, std::string>("Male","1"));
    sex_values.push_back(make_pair<std::string, std::string>("Female","0"));
}
...
userInfo::userInfo(const litesql::Database& db)
     : litesql::Persistent(db), id(Id), type(Type), userName(UserName), age(Age), sex(Sex) {
    defaults();
}
userInfo::userInfo(const litesql::Database& db, const litesql::Record& rec)
     : litesql::Persistent(db, rec), id(Id), type(Type), userName(UserName), age(Age), sex(Sex) {
    defaults();
...
userInfo::userInfo(const userInfo& obj)
     : litesql::Persistent(obj), id(obj.id), type(obj.type), userName(obj.userName), age(obj.age), sex(obj.sex) {
}
...
std::vector<litesql::Database::SchemaItem> WDNMD::getSchema() const {
    ...
    res.push_back(Database::SchemaItem("userInfo_","table","CREATE TABLE userInfo_ (id_ " + rowIdType + ",type_ " + backend->getSQLType(A_field_type_string,"") + "" +",userName_ " + backend->getSQLType(A_field_type_string,"") + "" +",age_ " + backend->getSQLType(A_field_type_integer,"") + "" +",sex_ " + backend->getSQLType(A_field_type_integer,"") + "" +")"));
